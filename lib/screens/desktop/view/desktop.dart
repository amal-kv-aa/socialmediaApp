    ...
    return Consumer<DesktopProvider>(builder: (context, value, child) {
      return 
       Scaffold(
        ...
        value.islarge ?const SizedBox():  Expanded(flex: 3, child: value.firstScreen),
          Expanded(flex:ResponsiveStyle.isTablet(context) ? 0: 3, child: Container())
        ],
      ),
            );
    });
...
class AccountDesktop extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Scaffold(
      ...
      body: 
      context.watch<LobyProvider>().currentIndex == 1 ?
     const ChatScreen() : const SizedBox()
      ,
    );
  }
}
