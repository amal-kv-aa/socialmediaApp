...
//=========================otp verify=============================//
  Future<String>? otpVerify(
      {required User otpModels, required String otp}) async {
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        HelperFunction().setAccesstocken(userData.token);
        HelperFunction().setuserId(userData.userDetails.id);
  ...
  Future<String>? googleSignIn(GoogleModel accountData) async {
    try {
      ...
      final response = await Dio()
          .post("${authbaseUrl}googleSignup", data: accountData.toJson());
      if (response.statusCode! >= 200 && response.statusCode! <= 299) {
